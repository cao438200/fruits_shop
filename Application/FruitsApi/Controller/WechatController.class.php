...
class WechatController extends Controller {
    ...
    {  
      	...
      	if(!$memberid){
      		$appid='wx25b01b557045ea0a';//公众号appid
			$redirect_uri = urlencode ( 'http://76c2xz.natappfree.cc/fruits_shop/index.php/FruitsApi/Wechat/getWxMember' );//获取openid页面
			$url ="https://open.weixin.qq.com/connect/oauth2/authorize?appid=$appid&redirect_uri=$redirect_uri&response_type=code&scope=snsapi_base&state=1#wechat_redirect";
      	}else{
      		$url="http://76c2xz.natappfree.cc/fruits_shop/index.php/FruitsApi/Wechat/dl";//有登陆信息直接跳转到指定页面
      	}
      	header("Location:".$url);	
    }  
    ...
    {  
        $appid = "wx25b01b557045ea0a";  
		$secret = "bb8e78a44d4e6dc236301cc6cf8b7635";  
		$code = $_GET["code"];
		//第一步:取全局access_token
		...
		if($member){
			...
			if($member['tpye']==1){
				$_SESSION['member_type']=1;//储层用户类型
			}
			$url="http://76c2xz.natappfree.cc/fruits_shop/index.php/FruitsApi/Wechat/zc1";
			header("Location:".$url);
		}else{
			...
			if($bool){
				$member=M('member')->field("Id,type")->Where("sWXOpenID='$openid'")->find();
				session_start();
				$_SESSION['memberid']=$member['Id'];//储层用户id
				$_SESSION['openid']=$openid;//储层用户openid
				if($member['tpye']==1){
				...
				$url="http://76c2xz.natappfree.cc/fruits_shop/index.php/FruitsApi/Wechat/zc2";
				header("Location:".$url);
			}else{
				echo '用户openid储层失败';
			}
